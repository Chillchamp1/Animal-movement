...
COORD = re.compile(
    r"^(x|y|lon|long|longitude|lat|latitude|utm[._]?[xy]|easting|northing|mu\.[xy]|coords?\.[xy]\d?)$"
)
TIME = re.compile(r"(time|date|timestamp|datetime|fixtime|acquisition|dt)")
IDCOL = re.compile(r"^(id|animal|animalid|animal_id|indiv|individual|collar|collar_id|uniqueid|tag)")
...
def describe(df: pd.DataFrame, label: str) -> None:
    ...
    id_cols, spp_cols = matching(cols, IDCOL), matching(cols, SPP)
    print(
